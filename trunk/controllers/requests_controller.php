...
'order' => array ($sort => $direction ) ); //'page' => $page,
	...
	function check() {
		$this->layout = 'ajax';
		//debug($this->data);
		if ($this->data ['Request'] ['begin_time'] >= $this->data ['Request'] ['end_time']) {
			echo 999;
			return;
		}
		echo $this->_check ( $this->data ['Request'] ['roomid'], $this->data ['Request'] ['date'], $this->data ['Request'] ['begin_time'], $this->data ['Request'] ['end_time'] );
	}
...
'conditions' => $conditions, 'limit' => $limit, 'order' => array ($sort => $direction ), 'page' => $page, 'recursives' => 0 );
	...
	function admin_bakking($id = null) {
		...
		//$this->log($hi['WebConfig']['punish_expense'],'test');
		$this->redirect ( array ('action' => 'index' ) );
	}
	 ...
	 * @param $begin string
	 * @param $end string
	 * @return int number of request between $begin and $end on $date
	 */
	private function _check($roomid, $date, $begin, $end) {
		$conditions = array ('Request.roomid' => ( int ) $roomid, 'Request.date' => date ( 'Y-m-d', strtotime ( $date ) ), 'OR' => array (array ('Request.begin_time >=' => $begin, 'Request.begin_time <' => $end ), array ('Request.end_time >' => $begin, 'Request.end_time <=' => $end ) ) );
