...
<form name="form1" action="" method="post">
    <div class="module_header">
        <div class="header_action">
            ...
            ?>
            <ul class="tabs">
                <li><?php echo $html->link(__('会議室タイプ追加', true), array('action' => 'admin_add'), array('title' => __('追加', true))); ?></li>
            </ul>
        </div>
    </div>
    <table class="tablesorter" cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th style="width: 20%" class="tableheader"><?php echo $this->Paginator->sort(__('名', true), 'RoomType.name'); ?></th>
                <th style="width: 60%" class="tableheader"><?php echo $this->Paginator->sort(__('説明', true), 'RoomType.description'); ?></th>
        ...
        <?php else: ?>
            <?php foreach ($list as $item) : ?>
                ...
                <tr<?php echo $class; ?>>
                    ...
                    <td align="center"><?php echo $form->checkbox('RoomType.SelectItem.' . ($stt - 1), array('value' => $item['RoomType']['id'], 'title' => __('選択 # ' . $stt, true), 'class' => 'cb_item')); ?></td>
                    <td align="left"><?php echo $item['RoomType']['name']; ?>&nbsp;</td>
                    <td align="left"><?php echo $item['RoomType']['description']; ?>&nbsp;</td>
                    <td align="center">
                        <?php
                        echo $html->image('admin_layout/icn_edit.png', array('url' => array('action' => 'admin_edit', $item['RoomType']['id']), 'title' => __('編集 # ' . $stt, true), 'alt' => 'edit'));
                        echo $html->image('admin_layout/icn_trash.png', array('url' => array('action' => 'admin_delete', $item['RoomType']['id']), 'title' => __('削除 # ' . $stt, true), 'alt' => 'delete', 'onclick' => "return confirm('" . __('本当に削除したいですか？', true) . "')"));
                        ?>
                    </td>
                </tr>
            <?php endforeach; ?>
